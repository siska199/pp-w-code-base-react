...
const Sidebar = () => {
  ...
  const handleChangeActiveMenu = (params: TParamsOnChangeMenu) => {
    isMaxMd && params?.groupMenu?.url && handleToggleSidebar();
  };
      ...
      <div id="container-sidebar" className=" h-full " onClick={handleToggleSidebar}>
        <div id="sidebar" onClick={handleStopPropagation} style={{ top: topPosition }} className={`fixed overflow-y-hidden h-[calc(100%-3rem)] left-0 bg-white w-0 ${showSidebar !== false && "md:w-[17rem]"}`}>
          <div className="p-8 w-full  flex flex-col gap-4 relative  h-full">
            ...
            <NestedMenu setting={setting} setSetting={setSetting} menu={listGroupingMenuSidebar} onChangeMenu={handleChangeActiveMenu} />
          </div>
        </div>
      </div>
      <div id="overlay-sidebar" className={`md-overlay min-h-screeen`} />
    </>
  );
};
