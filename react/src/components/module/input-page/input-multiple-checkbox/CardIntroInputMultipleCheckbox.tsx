...
const CardIntroInputMultipleCheckbox = () => {
  const initialForm = {
    "input-multi-checkbox": {
      value: ["alice-johnson label", "bob-smith label", "charlie-brown label"],
    },
  };
  ...
  const listExample = [
    {
      component: <CardVariantComp title={"default (unchecked state) "} withBorder={false} Component={<InputMultipleCheckbox onChange={handleOnChange} {...form["input-multi-checkbox"]} options={options?.listInputSelect} />} />,
    },
    {
      component: (
        <CardVariantComp
          title={"with label (checked state)"}
          withBorder={false}
          Component={<InputMultipleCheckbox onChange={handleOnChange} {...form["input-multi-checkbox"]} options={options?.listInputSelect?.map((data) => ({ ...data, value: `${data?.value} label` }))} label={"Input select with label"} />}
        />
      ),
    },
    {
      component: (
        <CardVariantComp
          title={"with error"}
          withBorder={false}
          Component={<InputMultipleCheckbox onChange={handleOnChange} {...form["input-multi-checkbox"]} options={options?.listInputSelect?.map((data) => ({ ...data, value: `${data?.value} error` }))} label={"Input select with error"} errorMessage="Error State : )" />}
        />
      ),
    },
  ];
