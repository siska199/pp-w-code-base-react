...
import InputSelectMakyo from '@components/ui/inputs/InputSelectMakyo';
...
const CardIntroInputSelect = () => {
  const { form, handleOnChange } = useForm<'input-select' | 'input-select-multiple' | 'input-select--select-all' | 'input-select-makyo'>({
    initialForm: {
      'input-select': {
        value: '',
      },
  ...
  const listExample = [
    ...
    {
      component: <CardVariantComp title={'Multiple Select (Makyo)'} withBorder={false} Component={<InputSelectMakyo onChange={handleOnChange} {...form['input-select-makyo']} options={options?.listOption} multiple={true} outlined={true} />} />,
    },
  ];
