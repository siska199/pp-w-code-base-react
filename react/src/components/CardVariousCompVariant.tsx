...
const CardVariousCompVariant = <TV extends object, TG extends string = string>(props: TProps<TV, TG>) => {
  ...
  return (
    <div className={`${customeClass?.container} flex flex-col flex-wrap w-full gap-2`}>
      {groups?.map((group, i) => (
        <div key={i} className={`${customeClass?.containerGroupVariant} flex flex-col flex-grow  gap-2 `}>
              ...
              ?.map(([key]) => {
                return (
                  <CardVariantComp
                    ...
                    customeClass={{
                      container: `${customeClass?.containerCardVariant} flex-basis flex-grow `,
                    }}
                  />
                );
              })}
