...
const Image = (props: TProps) => {
    const { src, alt, className, onLoadCustome, isLoadingImage = true, useSkeleton, customeClassName, withOverlay, overlayContent, ...attrs } = props;
    const [isLoading, setIsLoading] = useState<boolean>(isLoadingImage || true);
    ...
    return (
        <div className={cn({
            ...
            />
            {
                isLoading && useSkeleton && <div role="status" className=" absolute top-0 left-0 border h-full w-full border-gray-400 rounded shadow animate-pulse  ">
