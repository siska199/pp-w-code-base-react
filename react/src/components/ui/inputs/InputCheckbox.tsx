...
const InputCheckbox = (props: TProps) => {
  const { name, onChange, value, customeClassnameCheckbox, label = '', checked, withCheckbox,searchQuery, ...attrsInput } = props;
  ...
  return (
    <Container onMouseDown={(e) => e.preventDefault()} variant={'hsc'} gap={'base'} className={`${!label && '!w-fit'} ${customeClassnameCheckbox?.container} ${updatedChecked && '!bg-teald-50'} `}>
      ...
      {label && withCheckbox && (
        <label className={`mt-px `} htmlFor="radio">
          {label}
        </label>
      )}
    </Container>
